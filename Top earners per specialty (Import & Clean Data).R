...
#Get the number of unique IDs (each representing a distinct physician) receiving a payment
n_physicians <- x[, id] %>% uniqueN
...
top <- x %>%
  ...
  summarise(
    ...
    top0.1mean = sum(amount[rank(desc(amount)) <= round(0.001 * n())])/(0.001 * n()), #Get the mean amount by the top 0.1% (top 1 in 1000)
    top_mid_ratio = round(100*(top0.1mean/median)), #Ratio between the top-earning and median-earning physician(s)
    total_n = n(), #Total N of physicians per specialty
    top0.1_n = round(0.001 * n()) #N of physicians in the top 0.1% of each specialty
  )

#Save the file
qsave(top, "Top earners data.RData")
top <- qread("Top earners data.RData")
